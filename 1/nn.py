...
def stock():
    ...
    width = stock_days*2
    a, b, history = neural_network(get_stock_data(1), depth, width, True, True)
...
def yoga():
    data = get_yoga_data(1)
    # width = yoga_count*2
    for width in [*range(int(yoga_count/4), int(yoga_count*3)+1, int(yoga_count/4))]:
        print('WIDTH IS ', width)
        depths = [*range(1, 6)]
        util.handle_training_testing_plot(lambda depth: neural_network(data, depth, width, False), depths, f"Yoga NN {width} Depth vs Accuracy", 'Depth')
    ...
    # width = widths[0]
    # a, b, history = neural_network(get_yoga_data(1), depth, width, False, True)
    # epochs = [*range(1,history.params['epochs']+1)]
    # training_accuracies = history.history[accuracy_key]
    # testing_accuracies = history.history['val_'+accuracy_key]
